document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- FUNKCJE POMOCNICZE ---
    const saveWorkouts = () => localStorage.setItem('workouts', JSON.stringify(workouts));
    const saveTemplates = () => localStorage.setItem('workoutTemplates', JSON.stringify(workoutTemplates));
    const formatDateKey = (date) => `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}-${String(date.getDate()).padStart(2, '0')}`;
    
    const migrateData = () => {
        let needsSave = false;
        for (const key in workouts) {
            if (Array.isArray(workouts[key])) {
                workouts[key] = { exercises: workouts[key], note: '' };
                needsSave = true;
            }
            if (workouts[key].exercises) {
                workouts[key].exercises.forEach(ex => {
                    if(ex.reps && typeof ex.reps !== 'object') { ex.reps = { target: ex.reps, done: 0 }; needsSave = true; }
                    if(ex.sets && typeof ex.sets !== 'object') { ex.sets = { target: ex.sets, done: 0 }; needsSave = true; }
                    if(ex.km && typeof ex.km !== 'object') { ex.km = { target: ex.km, done: 0 }; needsSave = true; }
                });
            }
        }
        if (needsSave) saveWorkouts();
    };

    /**
     * Sprawdza, czy trening danego dnia został w pełni wykonany.
     * @param {string} dateKey - Klucz daty w formacie 'RRRR-MM-DD'.
     * @returns {boolean|null} - true (zrobione), false (niezrobione), null (brak treningu).
     */
    function checkWorkoutCompletion(dateKey) {
        const dayData = workouts[dateKey];
        if (!dayData || !dayData.exercises || dayData.exercises.length === 0) {
            return null; // Brak zaplanowanego treningu
        }

        for (const exercise of dayData.exercises) {
            if (exercise.reps && exercise.reps.done < exercise.reps.target) {
                return false; // Niezrobione
            }
            if (exercise.sets && exercise.sets.done < exercise.sets.target) {
                return false; // Niezrobione
            }
            if (exercise.km && exercise.km.done < exercise.km.target) {
                return false; // Niezrobione
            }
        }

        return true; // Wszystko zrobione
    }
    ...
    function updateProgress(index, type, value) {
        ...
        saveWorkouts();
        renderDayView();
        renderCalendar(); // Odśwież kalendarz, aby zaktualizować kolor dnia
    }
    ...
    // --- NOWA LOGIKA EDYCJI I USUWANIA SZABLONÓW ---
    ...
    function dayHasTemplate(dateKey, templateIndex) {
        ...
        const dayData = workouts[dateKey];
        if (!template || !dayData || !dayData.exercises) {
            return false;
        }
    ...
    function toggleTemplateOnDay(date) {
        ...
        if (hasTemplateAlready) {
            ...
            if (workouts[dateKey] && workouts[dateKey].exercises) {
                workouts[dateKey].exercises = workouts[dateKey].exercises.filter(
                    ex => !templateExerciseNames.has(ex.name)
                );
                if (workouts[dateKey].exercises.length === 0 && !workouts[dateKey].note?.trim()) {
                    delete workouts[dateKey];
                }
